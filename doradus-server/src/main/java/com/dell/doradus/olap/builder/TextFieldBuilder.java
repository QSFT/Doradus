...
public class TextFieldBuilder {
	...
	public TextFieldBuilder(IdsBuilder ids, ValuesBuilder vals) {
		m_ids = ids;
		m_vals = vals;
	...
	public void flush(FieldWriterSV writer) {
		for(int i = 0; i < size(); i++) {
			int id = m_docs.get(i);
			int doc = m_ids.doc(id);
			int val = m_values.get(i);
			int term = m_vals.term(val);
			writer.set(doc, term);
		}
	}
	...
	public void flush(FieldWriter writer) {
		...
		for(int i = 0; i < size(); i++) {
			int doc = m_ids.doc(m_docs.get(i));
			int term = m_vals.term(m_values.get(i));
			values[offsets[doc]] = term;
			offsets[doc]++;
		}
		for(int i = 0; i < docs; i++) {
