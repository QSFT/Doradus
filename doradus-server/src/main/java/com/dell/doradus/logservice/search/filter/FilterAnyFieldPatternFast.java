...
public class FilterAnyFieldPatternFast implements IFilter {
    private BSTR m_upper;
    private BSTR m_lower;
    private StrRef m_value = new StrRef();
    
    public FilterAnyFieldPatternFast(String pattern) {
    ...
    @Override public boolean check(ChunkReader reader, int doc) {
        for(int i = 0; i < reader.fieldsCount(); i++) {
            ...
            if(m_value.matchesPattern(m_upper, m_lower)) return true;
        }
        return false;
    }
