...
public class TaskDBUtils {
    private static final String TASK_CLAIM_ROW_PREFIX = "_claim";
	...
	public static TaskTable getTasksSchedule() {
		...
		try {
		    // TODO: Need to test for multi-tenant features
		    for (Tenant tenant : DBService.instance().getTenants()) {
		        for (ApplicationDefinition appDef : SchemaService.instance().getAllApplications(tenant)) {
		            ...
		            for (Map.Entry<String, String> schedule : taskSchedules.entrySet()) {
		                ...
		                if (schedValue != null) {
		                    // Check status
		                    TaskStatus status = getTaskStatus(tenant, appName, taskId);
	 ...
	 * 
	 * @return	Map of tenant names to application names to registered task names.
	 */
	public static Map<String, Map<String, Set<String>>> getAllTaskNames() {
	    Map<String, Map<String, Set<String>>> allTaskNames = new HashMap<>();
        for (Tenant tenant : DBService.instance().getTenants()) {
            Map<String, Set<String>> appTaskNameMap = new HashMap<>();
            allTaskNames.put(tenant.getKeyspace(), appTaskNameMap);
            for (ApplicationDefinition appDef : SchemaService.instance().getAllApplications(tenant)) {
                String appName = appDef.getAppName();
                Set<String> taskSet = new HashSet<>();
                appTaskNameMap.put(appName, taskSet);
                Iterator<DRow> taskRows = DBService.instance().getAllRowsAllColumns(tenant, SchemaService.TASKS_STORE_NAME);
	 ...
	 * 
	 * @param tenant    Tenant in which application resides.
	 * @param appName	Application name
	 * @param taskId	Task key
	 * @return			Task status (undefined if no status was actually found)
	 */
	public static TaskStatus getTaskStatus(Tenant tenant, String appName, String taskId) {
	...
	public static Set<TaskId> getLiveTasks(Tenant tenant) {
		return getLiveTasks(tenant, null);
	}
	 ...
	 */
	public static Set<TaskId> getLiveTasks(Tenant tenant, String appName) {
	    assert appName != null;
	 ...
	 */
	public static void setTaskStatus(Tenant tenant, String appName, String taskId, TaskStatus status) {
	 ...
	 */
	public static boolean claim(Tenant tenant, String appName, String taskId, long scheduledAt) {
		String store = SchemaService.TASKS_STORE_NAME;
		String rowKey = TASK_CLAIM_ROW_PREFIX + "/" + appName + "/" + taskId;
	...
	public static void startTask(DoradusTask task) {
		TaskStatus status = getTaskStatus(task.getTenant(), task.getAppName(), task.getTaskId());
		...
		setTaskStatus(task.getTenant(), task.getAppName(), task.getTaskId(), status);
	}	// startTask
	 ...
	 */
	public static boolean interruptTask(Tenant tenant, String appName, String taskId) {
		TaskStatus status = getTaskStatus(tenant, appName, taskId);
		boolean interrupted = status.getLastRunState() == TaskRunState.Started;
		if (interrupted) {
			status.setLastRunState(TaskRunState.Interrupting);
			setTaskStatus(tenant, appName, taskId, status);
		}
		return interrupted;
	}	// interruptTask
	 ...
	 * @param taskId	Task name
	 */
	public static void suspendTask(Tenant tenant, String appName, String taskId) {
		TaskStatus status = getTaskStatus(tenant, appName, taskId);
	 ...
	 * @param taskId	Task name
	 */
	public static void resumeTask(Tenant tenant, String appName, String taskId) {
		TaskStatus status = getTaskStatus(tenant, appName, taskId);
	...
	public static void deleteAppTasks(ApplicationDefinition appDef) {
		...
		while (rowIter.hasNext()) {
			...
			if (rowKey.startsWith(appName + "/") ||
				rowKey.startsWith(TASK_CLAIM_ROW_PREFIX + "/" + appName + "/")) {
