...
/**
 * Implements the REST command: GET /{application}/{table}/_statistics/{stat}?{params}.
 * Returns the requested parameters of the given statistic as a UNode tree.
 */
public class QueryStatCmd extends UNodeOutCallback {
    ...
    public UNode invokeUNodeOut(UNode inNode) {
        ...
        String stat = m_request.getVariableDecoded("stat");
        StatisticDefinition statDef = tableDef.getStatDef(stat);
        Utils.require(statDef != null, "Unknown statistic for table '%s': %s", table, stat);
        
        String params = m_request.getVariable("params");    // leave encoded
        StatResult statResult = SpiderService.instance().queryStatisticURI(appDef, statDef, params);
        return statResult.toDoc();
    }   // invokeUNodeOut 
