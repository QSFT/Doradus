...
/**
 * Implements the REST command: PUT /{application}/{table}/_statistics/{stat}/_refresh. A
 * task is queued to refresh the given statistic.
 */
public class StatsRefreshStatCmd extends RESTCallback {
    ...
    protected RESTResponse invoke() {
        ...
        return SpiderService.instance().refreshStatistic(application, statDef) ?
        		new RESTResponse(HttpCode.OK) :
        		new RESTResponse(HttpCode.SERVICE_UNAVAILABLE,
        				"Statistics refreshening is already started on " + stat);
    }
