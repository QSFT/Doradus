...
public class InverseXLinkQueryAll implements Query, XLinkQuery {
	private InverseXLinkQueryAny m_anyExists; 
	private InverseXLinkQueryAny m_hasFalse; 
	
	public InverseXLinkQueryAll(XLinkContext ctx, TableDefinition tableDef, LinkQuery lq) {
		m_anyExists = new InverseXLinkQueryAny(ctx, tableDef, new LinkQuery(LinkQuery.ANY, lq.link, lq.innerQuery, lq.filter));
		m_hasFalse = new InverseXLinkQueryAny(ctx, tableDef, new LinkQuery(LinkQuery.ANY, lq.link, new NotQuery(lq.innerQuery), lq.filter));
	}
